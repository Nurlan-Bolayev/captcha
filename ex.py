    ...
    class CheckCap:
        ...
        def set_token(self):
            if not app.config.get('captcha_token') :
                 r = requests.post(url = self.token_url, json=self.token_request_body) 
                 json_data = r.json()
                 app.config['captcha_token_settime'] = datetime.datetime.today().now()
                 app.config['captcha_token'] = json_data['token']
            else :
                 now = datetime.datetime.now()
                 time_diff = (now - app.config['captcha_token_settime']).total_seconds()
    ...
    class CheckCap:
        ...
        def set_token(self):
            if not app.config.get('captcha_token') :
                 ...
                 now = datetime.datetime.now()
                 print(now)
                 print(app.config['captcha_token_settime'])
